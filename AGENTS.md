...
Linear (team Fábrica): update the issue in the same turn the role finishes. In Progress = implementing. In Review = reviewing and testing, with evidence on the issue. Done = merged to `main` and verified; if the change ships on the Cloudflare Worker, Done only after the tag's target environment is live (staging for prerelease tags, production for closed tags). Labels `env:staging` / `env:production` record Worker environment (SHA + worker name + URL in the comment). Local-only work (CLI, tray, local API) can be Done without `env:*`. Never apply those labels on an unpushed commit. Chrome Web Store publication is never automatic. Canonical private docs: [Práticas operacionais no Linear](https://linear.app/djalmajr/document/praticas-operacionais-no-linear-08f51d46451e) and [Pinar — Aplicação das práticas](https://linear.app/djalmajr/document/pinar-aplicacao-das-praticas-operacionais-no-linear-659c156123a1).

Cloudflare releases use an environment-specific Vite artifact. For each environment, build with `CLOUDFLARE_ENV=<staging|production> bun run build:server`, inspect `apps/server/dist/server/wrangler.json` for the matching `targetEnvironment`, Worker, bindings, and routes, then run `bunx wrangler deploy --env <environment>` from `apps/server`. Never reuse a build from another environment or bypass a mismatch. The Deploy Worker workflow applies pending D1 migrations (`wrangler d1 migrations apply`) right before each `wrangler deploy`; for a manual deploy, apply authorized D1 migrations first. Closed tags (`vX.Y.Z`) deploy production and the desktop app via CI; prerelease tags (`vX.Y.Z-rc.1`, `vX.Y.Z-0`, alpha, beta) deploy staging only. Record the Worker version IDs and public health checks.

The product version is the root `package.json` version, and it is the only one: git tags (`vX.Y.Z` closed, `vX.Y.Z-<pre>` prerelease), release notes (`apps/server/src/lib/release-content.ts` + `release-locales/*` for closed tags), `/api/health`, the site footer and Settings → About all read it. Cut a release with `bun run release <patch|minor|major>` or `bun run release 1.0.0-rc.1`: it requires a clean tree; closed tags also need notes in all seven languages; it writes the version to `apps/cli`, `apps/tray` and `apps/server` too, commits `chore: release vX.Y.Z` and tags. It does not push. Pushing the tag is what deploys: closed → production Worker + macOS/Windows GitHub Latest; prerelease → staging Worker. `CLOUDFLARE_ENV=production bun run build:server` refuses to build unless HEAD is exactly on a closed release tag with a clean tree (`bun run release:check` shows why); staging and local builds are not gated, and a build ahead of the last tag shows as `X.Y.Z+<sha>` in the footer and About. The extension stays on `ext-vX.Y.Z` and never owns GitHub Latest. Worker CI needs `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` repository secrets.
