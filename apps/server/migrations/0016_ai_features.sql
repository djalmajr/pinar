-- AI features beyond the session summary share the credit ledger. SQLite cannot
-- widen a CHECK in place, so the usages table is rebuilt (same pattern as 0014).
-- `voice_pin` is reserved now so the next feature does not rebuild it again.
...
-- Copy before the triggers exist: a reserved row must not consume its grant twice.
...
-- "Save as component" remembers the stack the account exports to.
ALTER TABLE owner_preferences ADD COLUMN component_target TEXT
  CHECK (component_target IN ('html', 'react-tailwind', 'preact-htm'));

-- A collection keeps the last design system extracted from its pins.
ALTER TABLE collections ADD COLUMN design_system_json TEXT;
