-- Keep historical monthly grants readable while making the current Pro grant
-- a one-time allocation identified by one stable source id per account.
...
-- The column remains for backwards-compatible reads of older databases, but it
-- no longer schedules any refill.
UPDATE users SET ai_credit_refill_at = NULL;
