...
const locale = {
  ...
  screenshots: {
    ...
    "capture-review": {
      alt: "带有元素标注的页面，可供 Pinar 的 AI 功能审查和增强。",
      caption: "元素标注会保留诊断、组件生成和设计系统提取所需的已捕获结构。",
    },
    "capture-copy-failed": {
  ...
  articles: {
    ...
    "local-or-cloud": {
      ...
      sections: [
        ...
        },
        {
          heading: "云",
          paragraphs: [
            "云模式可实现远程工作区访问、托管保留期、Pinar Cloud AI 摘要、计费以及未列出的分享链接。本地 AI 和 BYOK 仍可使用，且不会消耗 Pinar Cloud 积分。在任何内容被远程存储之前，您需要接受当前政策。",
          ],
        },
        {
    ...
    "pins-and-comments": {
      ...
      sections: [
        ...
        },
        {
          ...
          bullets: [
            "区域图钉没有结构，因此「诊断」和「保存为组件」在其上保持禁用。",
    ...
    "send-to-agent": {
      ...
      sections: [
        ...
        },
        {
          heading: "诊断图钉并保存为组件",
          paragraphs: [
            "在已捕获结构的图钉上，「诊断」会请 AI 给出您所评论问题的可能原因和一个建议的 CSS 修复，并附置信度。您可以接受、编辑或丢弃该建议；只有已接受的诊断会随图钉保留并随数据包一起复制。使用 Pinar Cloud AI 时，一次诊断费用为 3 积分。",
            "「保存为组件」会把捕获的元素转换为适用于您技术栈的独立组件：HTML + CSS、React + Tailwind 或 Preact + htm。结果会列出文件、依赖和保真度说明，并在原始截图旁提供预览。您可以复制文件、下载 ZIP，或在 StackBlitz 中打开该组件。使用 Pinar Cloud AI 时费用为 10 积分，所选技术栈会为下一个图钉记住。",
          ],
          bullets: [
            "这两个操作都需要元素结构；区域图钉以及在此版本之前捕获的图钉无法使用它们。",
            "低置信度的诊断是有待核实的假设，而不是可以当作事实粘贴的结论。",
          ],
        },
      ],
    },
    ...
    "ai-features": {
      title: "AI 功能及测试方法",
      summary: "在一个位置查看全部 AI 功能、使用前提，以及本地 AI、BYOK 或 Pinar Cloud 的完整测试流程。",
      sections: [
        {
          heading: "先选择提供方",
          paragraphs: [
            "在本地服务器中，打开设置 → AI 助手，选择本地 AI 或 BYOK，填写兼容 OpenAI 的端点和模型，然后使用测试并保存。本地 AI 和 BYOK 不使用 Pinar 积分。在 Pinar Cloud 中，符合条件的已登录账户可以使用 AI；操作旁的问号图标会显示当前的 Pinar Cloud 积分消耗。",
          ],
        },
        {
          heading: "会话摘要",
          paragraphs: [
            "点击截图缩略图打开已保存的会话。在查看器顶部选择总结标注。结果会使用页面标题、URL 和标注评论。",
          ],
        },
        {
          heading: "标注诊断和组件生成",
          paragraphs: [
            "点击页面元素创建新的元素标注，结束会话，打开缩略图并在查看器中选择该标注。标注对话框会显示诊断和保存为组件，可输出 HTML、React + Tailwind 或 Preact + HTM。",
            "两项功能都需要带有已捕获结构的元素标注。区域标注和缺少结构的旧标注会显示前提提示；请使用当前扩展重新创建元素标注进行测试。",
          ],
        },
        {
          heading: "自动化复现",
          paragraphs: [
            "打开 Pinar 捕获工具栏并按 `G` 开始录制。正常操作页面，再次打开 Pinar，添加标注，并在 macOS 上按 `Command+Enter`，或在 Windows 和 Linux 上按 `Alt+Enter` 完成。打开已保存的会话：复现区域会列出操作，生成步骤和测试会创建文字步骤与 Playwright 测试。",
          ],
        },
        {
          heading: "设计系统提取",
          paragraphs: [
            "把同一网站页面中的至少三个新元素标注发送到同一集合，然后在集合菜单中选择提取设计系统。只有包含已捕获元素结构的标注会被计入；截图、区域标注以及缺少结构的旧标注都不符合条件。对话框会在提取前显示可用数量。",
            "生成结果会整理颜色、排版、间距、圆角、阴影和字体，并可导出为 W3C tokens、CSS 变量、Tailwind 主题或 DESIGN.md。",
          ],
        },
      ],
    },
    ...
    "nested-collections": {
      ...
      sections: [
        ...
        },
        {
          heading: "提取集合的设计系统",
          paragraphs: [
            "在集合的菜单中，「提取设计系统」会读取该集合中图钉的结构，并推导出它们共有的令牌：颜色、排版、间距、圆角和阴影，以及站点标识。它需要来自同一站点且已捕获结构的至少三个图钉。使用 Pinar Cloud AI 时费用为 15 积分。",
            "结果会以对话框打开，显示样本数量和任何警告，例如无法构成刻度的零散值。可将其导出为 CSS 变量、Tailwind 主题、W3C 设计令牌或 DESIGN.md 文件，添加更多图钉后可再次提取。",
          ],
        },
      ],
    },
      ...
      sections: [
        {
          heading: "套餐形态",
          paragraphs: [
            "Free 包含永久本地使用、250 MB 云配额和七天云保留期。Pro 按月或按年计费，提供 5 GB 和每月补充的 200 个不结转 AI 积分。",
          ],
        },
        {
          heading: "计费与可用性",
    ...
    "ai-credits": {
      title: "Pinar Cloud AI 摘要与积分",
      summary: "了解 Pinar Cloud 积分何时被预留、消耗、补充或退还。",
      sections: [
        {
          heading: "摘要费用",
          paragraphs: [
            "在 Pinar Cloud 中，会话摘要会在模型推理前预留 1 AI 积分。成功后预留被消耗。失败或中止的推理会立即退还；超过五分钟仍未结算的预留会自动退还。摘要限制为每个账户每分钟 10 次请求、每个 IP 每分钟 30 次；同一会话的重复请求会等待活动请求完成。",
          ],
        },
        {
          heading: "各 AI 功能的费用",
          paragraphs: [
            "Pinar Cloud 的每个 AI 功能都会在模型运行前预留积分，并在结果不可用时退还，与摘要完全一致。费用按请求固定，而不是按 token：会话摘要 1 积分，图钉诊断 3 积分，复现（文字步骤加 Playwright 测试）5 积分，将图钉保存为组件 10 积分，提取合集的设计系统 15 积分。",
            "技术证据和元素结构由扩展在没有模型的情况下捕获，不产生费用。无法运行的请求（没有捕获结构的图钉、同一域名快照少于三个的合集）会在预留任何积分之前被拒绝。",
          ],
        },
        {
          heading: "余额",
          paragraphs: [
            "购买的 Pinar Cloud 积分包会增加 1,000 积分。Pro 每月 200 积分额度不结转。账户菜单显示活动余额和下一次适用的补充日期。",
          ],
        },
        {
          ...
          paragraphs: [
            ...
            "包含的每月积分会先于购买包使用，且最快过期的余额会最先使用。购买的 1,000 积分包最长可用 12 个月。账户菜单会为有效的 Pro 账户显示剩余积分和下一次补充日期。当工作区语言属于七种受支持语言之一时，摘要会使用该语言。",
          ],
          bullets: [
      ...
      sections: [
        {
          heading: "配额与加购",
          paragraphs: [
            "Free 有 250 MB 基础云存储；Pro 有 5 GB。可选的 5 GB 和 20 GB 存储加购持续 12 个月，并在到期前七天和一天发送提醒邮件。截图上传必须是有效 PNG 文件，并在存储前通过原子配额检查。如果结果字节会超过当前配额，上传会暂停。",
          ],
        },
        {
          heading: "权益过期之后",
