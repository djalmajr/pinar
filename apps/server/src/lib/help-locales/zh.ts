...
const locale = {
  ...
  screenshots: {
    ...
    "capture-viewer": {
      alt: "Pinar 捕获查看器，显示带标注的截图、编号图钉、缩放控件和会话操作。",
      caption:
        "查看器集中显示已保存的截图、图钉评论和复制操作。",
    },
    "extension-options": {
  ...
  articles: {
    ...
    "copy-and-view": {
      title: "复制并查看捕获",
      summary:
        "在工作区中查看已保存的截图，并复制与其关联的上下文。",
      sections: [
        {
          heading: "查看器控件",
          paragraphs: [
            "捕获查看器支持指针平移、以光标为中心的滚轮缩放、双击缩放以及 50% 到 800% 的缩放控制。选择图钉后可查看渲染预览和原始 Markdown。",
          ],
          bullets: [
            "可从查看器下载截图或复制会话 Markdown。",
            "需要单独检查当前网站时，请使用原始页面链接。",
            "选择图钉可在预览和原始 Markdown 之间切换。",
            "分组会话会在同一个查看器中保留所有截图。",
            "原始页面链接会单独打开，不会更改已保存的会话。",
          ],
        },
        {
          heading: "从查看器复制",
          paragraphs: [
            "复制提示会写入所选捕获的关联 Markdown。共享可用时，打开提示 *.md 会打开公开 Markdown。",
          ],
        },
      ],
    },
