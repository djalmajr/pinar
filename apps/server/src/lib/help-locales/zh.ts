    ...
    "first-capture": {
      ...
      sections: [
        ...
        },
        {
          heading: "完成或取消会话",
          paragraphs: [
            "使用 `Command/Ctrl/Alt+Enter`：在 macOS 上按 `Command+Enter`，在其他系统按 `Ctrl+Enter`，或在任一系统按 `Alt+Enter`，即可完成并复制完整会话。Pinar 会明确确认成功。若失败，会显示可处理的错误，并保留会话供审阅和重试。放弃会话会在不复制的情况下结束。",
      ...
      sections: [
        {
          ...
          bullets: [
            ...
            "`G` 开始录制您在页面上执行的步骤。重新打开 Pinar，钉住结果，再用 `Command/Ctrl/Alt+Enter` 复制即可附上这些步骤；再次按 `G` 会丢弃录制。",
          ],
        },
        {
          heading: "焦点密集的页面",
    ...
    "send-to-agent": {
      ...
      sections: [
        ...
        },
        {
          heading: "诊断图钉并保存为组件",
          paragraphs: [
            "在已捕获结构的图钉上，「诊断」会请 AI 给出您所评论问题的可能原因和一个建议的 CSS 修复，并附置信度。您可以接受、编辑或丢弃该建议；只有已接受的诊断会随图钉保留并随数据包一起复制。一次诊断费用为 3 AI 积分。",
            "「保存为组件」会把捕获的元素转换为适用于您技术栈的独立组件：HTML + CSS、React + Tailwind 或 Preact + htm。结果会列出文件、依赖和保真度说明，并在原始截图旁提供预览。您可以复制文件、下载 ZIP，或在 StackBlitz 中打开该组件。费用为 10 AI 积分，所选技术栈会为下一个图钉记住。",
          ],
          bullets: [
    ...
    "nested-collections": {
      ...
      sections: [
        ...
        },
        {
          heading: "提取集合的设计系统",
          paragraphs: [
            "在集合的菜单中，「提取设计系统」会读取该集合中图钉的结构，并推导出它们共有的令牌：颜色、排版、间距、圆角和阴影，以及站点标识。它需要来自同一站点且已捕获结构的至少三个图钉，费用为 15 AI 积分。",
      ...
      sections: [
        {
          heading: "摘要费用",
          paragraphs: [
            "会话摘要会在模型推理前预留 1 AI 积分。成功后预留被消耗。失败或中止的推理会立即退还；超过五分钟仍未结算的预留会自动退还。摘要限制为每个账户每分钟 10 次请求、每个 IP 每分钟 30 次；同一会话的重复请求会等待活动请求完成。",
          ],
        },
        {
          heading: "各 AI 功能的费用",
          paragraphs: [
            "每个 AI 功能都会在模型运行前预留积分，并在结果不可用时退还，与摘要完全一致。费用按请求固定，而不是按 token：会话摘要 1 积分，图钉诊断 3 积分，复现（文字步骤加 Playwright 测试）5 积分，将图钉保存为组件 10 积分，提取合集的设计系统 15 积分。",
        ...
        },
        {
          heading: "余额",
          paragraphs: [
            "购买的积分包会增加 1,000 积分。Pro 每月 200 积分额度不结转。账户菜单显示活动余额和下一次适用的补充日期。",
          ],
        },
        {
