...
const locale = {
  ...
  screenshots: {
    ...
    "extension-options": {
      alt: "Pinar 扩展的偏好设置选项卡，包含本地和远程存储选项、语言及主题设置。",
      caption:
        "偏好设置选项卡用于选择存储位置，并设置整个扩展的语言和外观。",
    },
    "extension-preferences": {
      alt: "Pinar 扩展的捕获选项卡，显示精简或完整的智能体复制详情以及是否包含截图的开关。",
      caption:
        "捕获选项卡用于选择精简或完整交接，以及下次复制是否包含截图；保存后会在下一次复制前生效。",
    },
    "help-navigation": {
    ...
    "install-pinar": {
      alt: "Pinar 扩展的偏好设置选项卡，在本地服务器选项旁有下载 Pinar 按钮。",
      caption: "偏好设置选项卡在本地服务器旁提供 Pinar 应用下载，以便助手在这台电脑上启动。",
    },
    "options-local": {
      alt: "Pinar 扩展的偏好设置选项卡已选中本地服务器，捕获留在这台电脑上。",
    ...
    "preferences-privacy": {
      alt: "Pinar 扩展的捕获选项卡，显示可选循环指标以及要隐藏的额外 URL 键。",
      caption: "隐私偏好会把额外查询键从捕获的 URL 中去掉，并在你选择加入前保持循环指标关闭。",
    },
    "pricing-credits": {
      alt: "Pinar 定价页的 500 个 AI 点数加购卡片，含购买和十二个月有效期。",
  ...
  articles: {
    ...
    "local-or-cloud": {
      ...
      sections: [
        ...
        },
        {
          heading: "云",
          paragraphs: [
            "当已登录账户符合当前方案的资格时，云模式提供远程工作区访问、托管保留期、计费以及未列出的分享链接。本地应用和自托管服务器始终免费。Pinar Cloud 语音转写仍是 Pro 权益，不属于评估。本地 AI 和 BYOK 仍可使用，且不会消耗 Pinar Cloud 积分。在任何内容被远程存储之前，您需要接受当前政策。",
          ],
        },
        {
          ...
          paragraphs: [
            ...
            "接受现行的「条款」、「隐私」和「可接受使用」并确认邮箱验证码后，云存储才会开始。「Free」和 Pro 账户使用相同的登录流程。任何持有未列出 URL 的人都可以阅读分享链接。",
          ],
          bullets: [
            "本地工作区留在这台电脑上，不需要云账户。",
    ...
    "handoff-troubleshooting": {
      ...
      sections: [
        ...
        },
        {
          ...
          paragraphs: [
            "复制需要已保存的评论和至少一个图钉。工具栏显示「正在保存会话…」，隐藏叠加层，捕获截图，然后请离屏文档写入 text/html 和 text/plain。离屏文档先尝试 navigator.clipboard.write，再回退到 copy 事件加 execCommand。如果这次写入未成功，内容脚本仍会对返回的纯文本载荷尝试 writePlainText：clipboard.writeText，然后是隐藏文本框选择。",
    ...
    "account-and-sign-in": {
      ...
      sections: [
        {
          heading: "Free 和 Pro 的邮箱验证码",
          paragraphs: [
            "在扩展程序或 pinar.dev 输入邮箱。六位验证码十分钟后过期，输错五次后锁定。只有确认验证码后才会创建 Free 账户。新账户禁止使用临时邮箱服务商，允许自有域名。",
          ],
        },
        {
          heading: "会话",
        ...
        {
          heading: "从扩展程序的账户标签登录",
          paragraphs: [
            "打开账户标签，输入邮箱并请求验证码。输入收到的验证码，将扩展程序连接到您的账户。验证时，旧匿名安装中的捕获内容会迁移到该账户。",
            "未知或被阻止的邮箱会得到相同的请求响应，因此不会泄露账户是否存在。退出登录会结束当前扩展程序会话。",
          ],
          bullets: [
            "如果没有收到邮件，请稍后再试；验证码会过期，尝试过多会被延迟。",
            "请使用当前收到的邮箱验证码；扩展程序不再提供配对码。",
      ...
      sections: [
        {
          heading: "套餐形态",
          paragraphs: [
            "本地应用和自托管服务器始终免费。托管的 Pinar Cloud 遵循当前方案以及该账户的资格：新账户可能获得临时评估，较早的账户可能保留原有资格。没有适用于所有账户的统一期限。Pro 是付费年付方案，提供 2 GB，并在账户首次订阅时一次性提供 500 个 AI 点数。语音转写仍是 Pro 权益。定价页和账户权益显示当前适用的方案与配额。已经保存在云端的捕获仍受保留期和导出规则约束。",
          ],
        },
        {
          heading: "计费与可用性",
    ...
    "ai-credits": {
      ...
      sections: [
        ...
        },
        {
          heading: "余额",
          paragraphs: [
            "账户首次订阅 Pro 时一次性获得 500 点数。续订、取消后重新订阅或更换方案不会再次发放。购买的点数包增加 500 点数，有效期最长 12 个月。",
          ],
        },
        {
          heading: "重试语音转写并查看历史",
          ...
          bullets: [
            ...
            "如果工作区显示积分为零，请在购买另一个 500 积分包前先查看剩余余额。",
          ],
        },
      ],
    },
      ...
      sections: [
        {
          heading: "配额与加购",
          paragraphs: [
            "托管云配额是已登录账户当前包含的存储，加上仍有效的加购。Pro 包含 2 GB。1 GB 和 5 GB 存储加购有效期为 12 个月。到期时若超过配额，上传会暂停；超额内容享有30天宽限期，并可恢复至第90天，之后才可删除。能否购买加购取决于当前方案；请查看定价页和账户权益。",
          ],
        },
        {
          heading: "权益过期之后",
        ...
        },
        {
          ...
          paragraphs: [
            ...
            "已按 Free 权益存储且未标记为永久的云会话在 30 天后便有资格被清理。超过 Free 配额的 Pro 内容在付费资格结束后遵循 30 天宽限和 90 天恢复窗口。这台电脑上的仅本地历史永远不会被远程删除。资格并不承诺立即移除。",
          ],
          bullets: [
            "当新捕获暂停时，请通过删除会话或替换过重截图来腾出空间。仅当定价页和账户权益显示可以购买时，再购买 1 GB 或 5 GB 的十二个月加购。",
