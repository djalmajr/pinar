...
Available fixture profiles are `pro`, `founder`, and `lifetime`:
...
This gate starts an isolated runtime on port `17384` using `.wrangler/state/cloud-e2e`, authenticates through the real extension-code exchange, checks `runtime: "cloud"`, reads real D1 entitlements, and verifies the account menu in Chromium. It does not intercept account APIs.
...
- The Workers AI binding can call a remote service. Do not trigger real AI generation from this fixture unless that usage is intentional.
