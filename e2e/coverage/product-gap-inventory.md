...
| `workspace-patrocinio-somente-free` | Uma sessão paga ainda exibiu o bloco de patrocínio, embora sem os botões. A regra é ocultar a seção inteira para Pro mensal, Pro anual, Founder e Lifetime legado. | `ServerFooter.tsx` retorna `null` para qualquer sessão paga. | `tests/e2e/workspace/sponsorship.e2e.test.ts` cobre Free, `pro`, `founder` e `lifetime` legado. | Não implantada; staging ainda precisa de nova validação após entrega autorizada. |
